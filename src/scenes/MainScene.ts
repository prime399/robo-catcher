...
export default class MainScene extends Phaser.Scene {
	...
	editorCreate(): void {
		// This method is generally managed by the editor.
		// We'll move the creation logic to the create() method below.
	...
	create() {
		...
		// Add a platform for the robot to stand on
		const platform = this.add.image(width / 2, height - 50, "platform");
		...
		// Call editorCreate if it contains other essential setup managed by the editor
		// this.editorCreate(); // Uncomment if needed, but background creation is moved here.
	}
	...
	createMobileControls() {
		...
		this.add.text(buttonSize/2, controlsY, "←", textStyle).setOrigin(0.5).setScrollFactor(0);
		this.add.text(buttonSize*1.75, controlsY, "→", textStyle).setOrigin(0.5).setScrollFactor(0);
		this.add.text(width - buttonSize*1.75, controlsY, "↑", textStyle).setOrigin(0.5).setScrollFactor(0);
		this.add.text(width - buttonSize/2, controlsY, "D", textStyle).setOrigin(0.5).setScrollFactor(0);
		
		// Add a semi-transparent background for the controls
		this.add.rectangle(width/2, controlsY, width, buttonSize, 0x000000, 0.2)
			.setScrollFactor(0)
			.setDepth(-1); // Place behind the buttons
	}
	...
	updateParallaxBackground(scrollSpeed: number) {
		...
		const scaledWidth = width * bgScale;
		
		// Different speeds for each layer
		const speeds = [0.5, 1.0, 1.5, 2.0];
		
		// Update the background position
		this.bgX -= scrollSpeed;
